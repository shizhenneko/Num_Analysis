...
def f(x):
    return np.sin(np.pi * x) * np.log(2+x)

# 默认使用{1,x,x^2....}给出想要逼近的最高次数，以及内积区间
def optimal_approximation(function,n):
    ...
    for i in range(n+1):
        for j in range(n+1):
            ...
            A[i,j] = inner_product(phi_i, phi_j)

    for i in range(n+1):
        # v[i] = <f,\varphi_i> = ∫[a,b] f(x) * x^i dx
        phi_i = lambda x, power=i: x**power
        v[i] = inner_product(function, phi_i)
    
    # 解线性方程组 Ax = v，得到系数组x
    x = np.linalg.solve(A, v)
    return x

# 此处我们要定义函数的内积运算，即在[a,b]区间内的积分,这里使用[-1,1]的Gauss数值积分
def inner_product(f:Callable, g:Callable):
    integrand = lambda x: f(x) * g(x)
    n = 6
    nodes, weights = np.polynomial.legendre.leggauss(n)
    return sum(weights * integrand(nodes))
...
if __name__ == "__main__":
   ...
   plt.plot(x_plot, y_approx, 'r--', label=f'逼近多项式 P_{4}(x)', linewidth=2)
   plt.xlabel('x')
   plt.ylabel('y')
   plt.title(f'最佳平方逼近 (n=4, 区间=[-1,1])')
   plt.legend()
